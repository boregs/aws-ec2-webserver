...
resource "aws_s3_bucket_website_configuration" "s3_static_config" {
    bucket = aws_s3_bucket.aws-s3-static-website-bucket.id 
...
resource "aws_s3_bucket_policy" "s3_bucket_policy" {
    bucket = aws_s3_bucket.aws-s3-static-website-bucket.id # Links the policy to the S3 bucket
        ...
        Statement = [   # Opens the list of "rules" for the policy
            {
                ...
                Resource  = "${aws_s3_bucket.aws-s3-static-website-bucket.arn}/*"  # Defines to which resource the policy applies (the /* at the end means all objects inside the bucket)
            }
        ]
    })
}